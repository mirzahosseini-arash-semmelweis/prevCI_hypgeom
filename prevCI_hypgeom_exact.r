prevCI.hypgeom.exact <- function(N, n, n_pos, Se, Sp, conf.level = 0.95, alt = "two.sided") {
  # Description
  # Calculates exact Clopper-Pearson-type confidence interval (CI)
  ...
  if (Se < 0.5 | Se > 1 | Sp < 0.5 | Sp > 1) {stop("Se and Sp must be between 0.5 and 1.")}
  if (conf.level <= 0 | conf.level >= 1) {stop("Confidence level must be between 0 and 1.")}
  
  # give warning on computation time
  if (n >= 250 & n < 500) {message("This calculation may take more than a minute, consider using simulation-based or Bayesian alternative.")}
  if (n >= 500) {message("This calculation may take forever, consider using simulation-based or Bayesian alternative.")}
  ...
  get_pos_dist <- function(Nsick, from, to) {
    # hypergeometric distribution of true sick in sample
    true_sample_dist <- dhyper(0:n, Nsick, N - Nsick, n)
    
    # initialize all sample possibilities
    comb <- expand.grid(
      true_in_sample = 0:n,
      false_negatives = 0:n,
      false_positives = 0:n
    )
    # filter only sample combinations that are possible based on true sick in sample
    comb <- subset(comb,
                   false_negatives <= true_in_sample &
                   false_positives <= (n - true_in_sample))
    
    # calculate theoretical observed positives in sample combinations
    comb$observed_pos <- with(comb, true_in_sample + false_positives - false_negatives)
    
    # filter sample combinations with desired range of observed_pos
    comb <- subset(comb, observed_pos >= from & observed_pos <= to)
    
    # calculate the probabilties for observed_pos in each sample combination
    comb$prob <- with(comb,
                      true_sample_dist[true_in_sample + 1] *
                      dbinom(false_positives, n - true_in_sample, 1 - Sp) *
                      dbinom(false_negatives, true_in_sample, 1 - Se))
    pos_dist <- tapply(comb$prob, comb$observed_pos, sum)
  ...
  # look for MLE from (RG estimate - 1) to (RG estimate + 2), since MLE usually overestimates
  # true prevalence compared to RG
  low <- max(floor(N*RG) - 1, 0)
  ...
  if (alt == "two.sided" & LCL != UCL) {
    ...
    if (!exists("Blaker_LCL")) {
      Blaker_LCL <- Nsick_MLE # if none of values are accepted, use MLE as CI bound
    }
    ...
    if (!exists("Blaker_UCL")) {
      Blaker_UCL <- Nsick_MLE # if none of values are accepted, use MLE as CI bound
    }
...
################################################################################
# example
prevCI.hypgeom.exact(50, 25, 10, 0.9, 0.9, conf.level = 0.95, alt = "t")
N = 50
n = 25
n_pos = 10
Se = 0.9
Sp = 0.9
alpha = 0.025
tab <- matrix(ncol = N + 1, nrow = n + 1)
for (i in 1:(N+1)) {
  tab[,i] <- get_pos_dist(i-1,from = 0, to = n)
}
library(data.table)
fwrite(tab, "tab.csv")

prevCI.hypgeom.exact(1400, 171, 1, 1, 1, conf.level = 0.95, alt = "t")
